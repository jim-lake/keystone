...
var toolbar = require('./toolbar-58248136.cjs.prod.js');
require('./orderable-cf9aac62.cjs.prod.js');
var utils = require('./utils-5172a12a.cjs.prod.js');
var utils$1 = require('./utils-e1413915.cjs.prod.js');
var button = require('@keystone-ui/button');
var api = require('./api-085a9cfe.cjs.prod.js');
var formFromPreview = require('./form-from-preview-da999ce9.cjs.prod.js');
...
function findChildPropPathsWithPrevious(value, prevValue, schema, newPath, prevPath, pathWithKeys) {
  switch (schema.kind) {
    ...
    case 'conditional':
      ...
      return findChildPropPathsWithPrevious(value.value, hasChangedDiscriminant ? prevValue.value : utils$1.getInitialPropsValue(schema.values[value.discriminant]), schema.values[value.discriminant], newPath.concat('value'), hasChangedDiscriminant ? undefined : prevPath === null || prevPath === void 0 ? void 0 : prevPath.concat('value'), hasChangedDiscriminant ? undefined : pathWithKeys === null || pathWithKeys === void 0 ? void 0 : pathWithKeys.concat('value'));
    case 'object':
      {
        const paths = [];
    ...
    case 'array':
      {
        const paths = [];
        const prevKeys = utils$1.getKeysForArrayValue(prevValue);
        const keys = utils$1.getKeysForArrayValue(value);
        for (const [i, val] of value.entries()) {
          const key = keys[i];
          const prevIdx = prevKeys.indexOf(key);
          let prevVal;
          if (prevIdx === -1) {
            prevVal = utils$1.getInitialPropsValue(schema.element);
          } else {
...
function ComponentBlockRender(_ref2) {
  ...
  const getPreviewProps = React.useMemo(() => {
    return utils$1.createGetPreviewProps({
      kind: 'object',
      fields: componentBlock.schema
...
function propPathWithIndiciesToKeys(propPath, val) {
  return propPath.map(key => {
    ...
    const keys = utils$1.getKeysForArrayValue(val);
...
function ChromefulComponentBlockElement(props) {
  ...
  const isValid = React.useMemo(() => utils$1.clientSideValidateProp({
    kind: 'object',
...
function findArrayFieldsWithSingleChildField(schema, value) {
  const propPaths = [];
  utils$1.traverseProps(schema, value, (schema, value, path) => {
...
function withComponentBlocks(blockComponents, editorDocumentFeatures, relationships, editor) {
  ...
  const memoizedGetDocumentFeaturesForChildField = weakMemoize__default["default"](options => {
    return utils$1.getDocumentFeaturesForChildField(editorDocumentFeatures, options);
  });
  const {
    normalizeNode,
    deleteBackward,
      ...
      if (componentPropNode.type === 'component-inline-prop') {
        slate.Editor.withoutNormalizing(editor, () => {
          ...
          if (componentPropNode.propPath !== undefined && componentBlock !== undefined) {
            ...
            const ancestorFields = utils$1.getAncestorSchemas(rootSchema, componentPropNode.propPath, componentBlockNode.props);
            ...
            if (idx !== -1) {
              ...
              const val = utils$1.getValueAtPropPath(componentBlockNode.props, componentPropNode.propPath.slice(0, arrayFieldIdx));
    ...
    if (node.type === 'component-block') {
      ...
      if (componentBlock) {
        ...
        for (const [propPath, arrayField] of findArrayFieldsWithSingleChildField(rootSchema, node.props)) {
          ...
          const arrVal = utils$1.getValueAtPropPath(node.props, propPath);
          const prevKeys = utils$1.getKeysForArrayValue(arrVal);
          const prevKeysSet = new Set(prevKeys);
          const alreadyUsedIndicies = new Set();
          const newVal = [];
          const newKeys = [];
          const getNewKey = () => {
            let key = utils$1.getNewArrayElementKey();
            while (prevKeysSet.has(key)) {
              key = utils$1.getNewArrayElementKey();
            }
            return key;
          };
          for (const [, node] of nodesWithin) {
            ...
            if (arrVal.length <= idxFromValue || alreadyUsedIndicies.has(idxFromValue) && isEmptyChildFieldNode(node)) {
              newVal.push(utils$1.getInitialPropsValue(arrayField.element));
          ...
          utils$1.setKeysForArrayValue(newVal, newKeys);
          if (!areArraysEqual(arrVal, newVal)) {
            const transformedProps = utils$1.replaceValueAtPropPath(rootSchema, node.props, newVal, propPath);
            slate.Transforms.setNodes(editor, {
        ...
        const missingKeys = new Map(utils$1.findChildPropPaths(node.props, componentBlock.schema).map(x => [JSON.stringify(x.path), x.options.kind]));
        ...
        utils$1.findChildPropPaths(node.props, blockComponents[node.component].schema).forEach((x, index) => {
  ...
  if (schema.kind === 'object') {
    ...
    for (const [key, innerSchema] of Object.entries(schema.fields)) {
      ...
      if (innerValue === undefined) {
        hasChanged = true;
        newObjectValue[key] = utils$1.getInitialPropsValue(innerSchema);
        continue;
      }
...
function insertComponentBlock(editor, componentBlocks, componentBlock) {
  const node = utils$1.getInitialValue(componentBlock, componentBlocks[componentBlock]);
...
const BlockComponentsButtons = _ref => {
  ...
  const editor = utils.useStaticEditor();
...
const ComponentBlocksElement = _ref2 => {
  ...
  const selected = slateReact.useSelected();
  const [currentElement, setElement] = utils.useElementWithSetNodes(editor, __elementToGetPath);
  const {
    spacing
  } = core.useTheme();
  const blockComponents = React.useContext(ComponentBlockContext);
  ...
  const onRemove = utils.useEventCallback(() => {
  ...
  if (ancestorComponentProp) {
    ...
    if (ancestorComponent[0].type === 'component-block') {
      ...
      if (componentBlock && propPath) {
        const childField = utils$1.getSchemaAtPropPath(propPath, ancestorComponent[0].props, componentBlock.schema);
        if ((childField === null || childField === void 0 ? void 0 : childField.kind) === 'child') {
          return utils$1.getDocumentFeaturesForChildField(editorDocumentFeatures, childField.options);
        }
      }
    }
  }
...
const LinkElement = _ref => {
  ...
  const editor = utils.useStaticEditor();
  const [currentElement, setNode] = utils.useElementWithSetNodes(editor, __elementForGettingPath);
  ...
  const unlink = utils.useEventCallback(() => {
    slate.Transforms.unwrapNodes(editor, {
      at: slateReact.ReactEditor.findPath(editor, __elementForGettingPath)
    });
  });
  const forceValidation = utils.useForceValidation();
      ...
      if (node.type === 'component-block') {
        ...
        node.children.forEach((child, index) => {
          if (slate.Node.string(child) === '' && slate.Element.isElement(child) && (child.type === 'component-block-prop' || child.type === 'component-inline-prop') && child.propPath !== undefined) {
            ...
            const placeholder = utils$1.getPlaceholderTextForPropPath(child.propPath, componentBlocks[node.component].schema, node.props);
            if (placeholder) {
              decorations.push({
                placeholder,
                anchor: start,
