...
import { c as createAdminMeta } from './createAdminMeta-24be8f3a.esm.js';
import { allowAll } from '../access/dist/keystone-6-core-access.esm.js';
import { GraphQLSchema, parse, Kind, validate, execute, GraphQLNonNull, GraphQLList, print, graphql as graphql$1 } from 'graphql';
import { Q as QueryMode } from './next-fields-34f831a7.esm.js';
import { g as graphqlBoundToKeystoneContext, f as field, J as JSON } from './graphql-ts-schema-9020a95a.esm.js';
...
import { c as cannotForItem, m as mapUniqueWhereToWhere, r as resolveWhereInput, a as runWithPrisma, b as cannotForItemFields, d as resolveUniqueWhereInput, e as isRejected, f as isFulfilled, g as getOperationAccess, h as getAccessFilters, j as getWriteLimit, k as checkFilterOrderAccess, l as getDBFieldKeyForFieldOnMultiField, p as promiseAllRejectWithAllErrors, n as getPrismaNamespace, o as findOne, q as findMany, s as count, t as getVariablesForGraphQLField, u as executeGraphQLFieldToRootVal, v as initialiseLists, w as setWriteLimit, x as setPrismaNamespace } from './config-50249dc4.esm.js';
import { a as accessDeniedError, e as extensionError, b as accessReturnError, u as userInputError, v as validationFailureError, r as resolverError, c as relationshipError } from './graphql-errors-473725b1.esm.js';
import { arg, nonNull, list, inputObject, Int, Boolean } from '@graphql-ts/schema/api-without-context';
  ...
  return Object.fromEntries(Object.entries(data).flatMap(_ref6 => {
    ...
    if (dbField.kind === 'multi') {
      return Object.entries(value).map(_ref7 => {
        ...
        return [getDBFieldKeyForFieldOnMultiField(fieldKey, innerFieldKey), transformInnerDBField(dbField.fields[innerFieldKey], context, fieldValue)];
      });
    }
...
function getMutationsForList(list$1) {
  ...
  const createMany$1 = field({
    ...
    args: {
      data: arg({
        type: nonNull(list(nonNull(list$1.graphql.types.create)))
      })
    },
  ...
  const updateOne$1 = field({
    ...
    args: {
      where: arg({
        type: nonNull(list$1.graphql.types.uniqueWhere),
  ...
  const deleteMany$1 = field({
    ...
    args: {
      where: arg({
        type: nonNull(list(nonNull(list$1.graphql.types.uniqueWhere)))
      })
    },
  ...
  return {
    mutations: {
      ...(list$1.graphql.isEnabled.create && {
        [list$1.graphql.names.createMutationName]: createOne$1,
        [list$1.graphql.names.createManyMutationName]: createMany$1
      }),
      ...(list$1.graphql.isEnabled.update && {
        [list$1.graphql.names.updateMutationName]: updateOne$1,
        [list$1.graphql.names.updateManyMutationName]: updateMany$1
      }),
      ...(list$1.graphql.isEnabled.delete && {
        [list$1.graphql.names.deleteMutationName]: deleteOne$1,
        [list$1.graphql.names.deleteManyMutationName]: deleteMany$1
      })
    },
    updateManyInput
  };
...
function getQueriesForList(list$1) {
  ...
  const findMany$1 = field({
    type: list(nonNull(list$1.graphql.types.output)),
  ...
  const countQuery = field({
    ...
    args: {
      where: arg({
        type: nonNull(list$1.graphql.types.where),
        defaultValue: list$1.isSingleton ? {
          id: {
            equals: '1'
          }
...
function collectTypes(lists, updateManyByList) {
  ...
  collectedTypes.push(JSON.graphQLType);
...
function localImageAssetsAPI(storageConfig) {
  return {
    ...
    async upload(buffer, id, extension) {
      await fs__default.writeFile(path__default.join(storageConfig.storagePath, `${id}.${extension}`), buffer);
    },
    async delete(id, extension) {
      try {
        await fs__default.unlink(path__default.join(storageConfig.storagePath, `${id}.${extension}`));
      } catch (e) {
        const error = e;
...
function localFileAssetsAPI(storageConfig) {
  return {
    ...
    async upload(stream, filename) {
      const writeStream = fs__default.createWriteStream(path__default.join(storageConfig.storagePath, filename));
      ...
      try {
        ...
        const {
          size: filesize
        } = await fs__default.stat(path__default.join(storageConfig.storagePath, filename));
        return {
          filesize,
          filename
        };
      } catch (e) {
        await fs__default.remove(path__default.join(storageConfig.storagePath, filename));
        throw e;
      }
    },
    async delete(filename) {
      try {
        await fs__default.unlink(path__default.join(storageConfig.storagePath, filename));
      } catch (e) {
        const error = e;
        // If the file doesn't exist, we don't care
        if (error.code !== 'ENOENT') {
...
const defaultTransformName = filename => {
  ...
  const id = crypto.randomBytes(24).toString('base64').replace(/[^a-zA-Z0-9]/g, '').slice(12);
  ...
  return {
    ...
    getKeystone: prismaModule => {
      const prismaClient = new prismaModule.PrismaClient({
        log: config.db.enableLogging === true ? ['query'] : config.db.enableLogging === false ? undefined : config.db.enableLogging,
        datasources: {
          [config.db.provider]: {
            url: config.db.url
          }
