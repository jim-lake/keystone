...
async function withMigrate(schemaPath, cb) {
  ...
  try {
    ...
    const closePromise = new Promise(resolve => {
      const child = migrate.engine.child;
...
async function pushPrismaSchemaToDatabase(dbUrl, shadowDbUrl, schema, schemaPath, resetDb) {
  const created = await createDatabase(dbUrl, path__default.dirname(schemaPath));
  if (created) {
...
We need to reset the ${credentials.type} database "${credentials.database}" at ${getDbLocation(credentials)}.`);
    ...
    if (evaluateDataLossResult.migrationSteps) {
      ...
      const migrationName = slugify(migrationNameInput, {
        separator: '_'
      }).slice(0, 200);
...
export { deployMigrations as a, devMigrations as d, pushPrismaSchemaToDatabase as p, runMigrateWithDbUrl as r, withMigrate as w };
