...
var adminMetaGraphql = require('../../../dist/admin-meta-graphql-4908958f.cjs.prod.js');
var createSystem = require('../../../dist/createSystem-7a99e7f8.cjs.prod.js');
var artifacts = require('../../../dist/artifacts-e2a7df6a.cjs.prod.js');
var url = require('url');
...
async function build(cwd, _ref) {
  ...
  // TODO: this cannot be changed for now, circular dependency with getSystemPaths, getEsbuildConfig
  const config = artifacts.getBuiltKeystoneConfiguration(cwd);
  const {
    graphQLSchema,
    adminMeta
  } = createSystem.createSystem(config);
  const paths = artifacts.getSystemPaths(cwd, config);
  if (prisma) {
    if (frozen) {
      await artifacts.validatePrismaAndGraphQLSchemas(cwd, config, graphQLSchema);
      console.log('✨ GraphQL and Prisma schemas are up to date');
    } else {
      await artifacts.generatePrismaAndGraphQLSchemas(cwd, config, graphQLSchema);
      console.log('✨ Generated GraphQL and Prisma schemas');
    }
    await artifacts.generateTypescriptTypesAndPrisma(cwd, config, graphQLSchema);
  }
  ...
  await nextBuild__default["default"](paths.admin);
}
...
function printAbout() {
  console.log(`${chalk__default["default"].yellow('Keystone collects anonymous data when you run')} ${chalk__default["default"].green('"keystone dev"')}`);
  console.log();
...
function printTelemetryStatus() {
  ...
  if (telemetry === undefined) {
    console.log(`Keystone telemetry has been reset to ${chalk__default["default"].yellow('uninitialized')}`);
    console.log();
    console.log(`Telemetry will be sent the next time you run ${chalk__default["default"].green('"keystone dev"')}, unless you opt-out`);
  } else if (telemetry === false) {
    console.log(`Keystone telemetry is ${chalk__default["default"].red('disabled')}`);
    console.log();
    console.log(`Telemetry will ${chalk__default["default"].red('not')} be sent by this system user`);
  } else if (typeof telemetry === 'object') {
    console.log(`Keystone telemetry is ${chalk__default["default"].green('enabled')}`);
    console.log();
    ...
    for (const [projectPath, project] of Object.entries(telemetry.projects)) {
      console.log(`  Project telemetry for "${chalk__default["default"].yellow(projectPath)}" was last sent on ${project === null || project === void 0 ? void 0 : project.lastSentDate}`);
    }
    console.log();
    console.log(`Telemetry will be sent the next time you run ${chalk__default["default"].green('"keystone dev"')}, unless you opt-out`);
  }
...
function inform() {
  ...
  console.log(`${chalk__default["default"].bold('Keystone Telemetry')}`);
  printAbout();
  console.log(`You can use ${chalk__default["default"].green('"keystone telemetry --help"')} to update your preferences at any time`);
  console.log();
  console.log(`No telemetry data has been sent yet, but telemetry will be sent the next time you run ${chalk__default["default"].green('"keystone dev"')}, unless you opt-out`);
  console.log(); // gap to help visiblity
...
async function sendEvent(eventType, eventData) {
  const endpoint = process.env.KEYSTONE_TELEMETRY_ENDPOINT || createExpressServer.telemetryEndpoint;
...
const devLoadingHTMLFilepath = path__namespace["default"].join(createExpressServer.pkgDir, 'static', 'dev-loading.html');
...
async function dev(cwd, _ref) {
  ...
  const httpServer = app ? http.createServer(app) : null;
  let expressServer = null;
  let hasAddedAdminUIMiddleware = false;
  const configWithExtendHttp = artifacts.getBuiltKeystoneConfiguration(cwd);
  const config$1 = stripExtendHttpServer(configWithExtendHttp);
  const paths = artifacts.getSystemPaths(cwd, config$1);
  ...
  async function stop(aHttpServer) {
    ...
    if (exit) throw new utils.ExitError(1);
  }
  const initKeystone = async () => {
    var _configWithExtendHttp;
    await fs__namespace["default"].remove(paths.admin);
    const {
      adminMeta,
      graphQLSchema,
      context,
      ...
      apolloServer,
      ...rest
    } = await setupInitialKeystone(cwd, config$1, {
      server,
      prisma,
      dbPush
    });
    ...
    const nextApp = await initAdminUI(cwd, config$1, graphQLSchema, adminMeta, ui);
    if (nextApp && expressServer && context) {
      expressServer.use(createExpressServer.createAdminUIMiddlewareWithNextApp(config$1, context, nextApp));
    }
    hasAddedAdminUIMiddleware = true;
    initKeystonePromiseResolve();
    const initialisedLists = config.initialiseLists(config$1);
    const originalPrismaSchema = artifacts.printPrismaSchema(initialisedLists, config$1.db.prismaClientPath, config$1.db.provider, config$1.db.prismaPreviewFeatures, config$1.db.additionalPrismaDatasourceProperties, config$1.db.extendPrismaSchema);
    let lastPrintedGraphQLSchema = graphql.printSchema(graphQLSchema);
    let lastApolloServer = apolloServer || null;
    if (config$1.telemetry !== false) {
      runTelemetry(cwd, initialisedLists, config$1.db.provider);
    }
    for await (const buildResult of builds) {
      ...
      try {
        ...
        const newConfigWithHttp = artifacts.getBuiltKeystoneConfiguration(cwd);
        const newConfig = stripExtendHttpServer(newConfigWithHttp);
        if (prisma) {
          const newPrismaSchema = artifacts.printPrismaSchema(config.initialiseLists(newConfig), config$1.db.prismaClientPath, newConfig.db.provider, newConfig.db.prismaPreviewFeatures, newConfig.db.additionalPrismaDatasourceProperties, newConfig.db.extendPrismaSchema);
          ...
          if (JSON.stringify(newConfig.db.enableLogging) !== JSON.stringify(config$1.db.enableLogging) || newConfig.db.url !== config$1.db.url || newConfig.db.useMigrations !== config$1.db.useMigrations) {
        ...
        if (newPrintedGraphQLSchema !== lastPrintedGraphQLSchema) {
          await fs__namespace["default"].writeFile(paths.schema.graphql, artifacts.getFormattedGraphQLSchema(newPrintedGraphQLSchema));
          lastPrintedGraphQLSchema = newPrintedGraphQLSchema;
        }
        await artifacts.generateTypescriptTypes(cwd, newConfig, graphQLSchema);
        ...
        if (prismaClientModule) {
          if (server && lastApolloServer) {
            ...
            const servers = await createExpressServer.createExpressServer(newConfig, graphQLSchema, keystone.context);
            if (nextApp) {
              servers.expressServer.use(createExpressServer.createAdminUIMiddlewareWithNextApp(newConfig, keystone.context, nextApp));
            }
  ...
  if ((_config$server = config$1.server) !== null && _config$server !== void 0 && _config$server.healthCheck && app) {
    const healthCheckPath = config$1.server.healthCheck === true ? createExpressServer.healthCheckPath : config$1.server.healthCheck.path || createExpressServer.healthCheckPath;
  ...
  if (app && httpServer) {
    ...
    app.use((req, res, next) => {
      ...
      if (expressServer && pathname === (((_config$graphql = config$1.graphql) === null || _config$graphql === void 0 ? void 0 : _config$graphql.path) || '/api/graphql')) {
        return expressServer(req, res, next);
      }
    ...
    if (config$1 !== null && config$1 !== void 0 && config$1.server && 'port' in config$1.server) {
      httpOptions.port = config$1.server.port;
    }
    if (config$1 !== null && config$1 !== void 0 && config$1.server && 'options' in config$1.server && config$1.server.options) {
      Object.assign(httpOptions, config$1.server.options);
    }
    ...
    const server = httpServer.listen(httpOptions, err => {
      ...
      console.log(`⭐️ GraphQL API available at ${((_config$graphql2 = config$1.graphql) === null || _config$graphql2 === void 0 ? void 0 : _config$graphql2.path) || '/api/graphql'}`);
...
async function setupInitialKeystone(cwd, config, options) {
  ...
  if (prisma) {
    ...
    const prismaSchema = (await artifacts.generatePrismaAndGraphQLSchemas(cwd, config, graphQLSchema)).prisma;
    const prismaClientGenerationPromise = artifacts.generateTypescriptTypesAndPrisma(cwd, config, graphQLSchema);
    if (config.db.useMigrations) {
      await migrations.devMigrations(config.db.url, config.db.shadowDatabaseUrl, prismaSchema, paths.schema.prisma, false);
    } else if (dbPush) {
      await migrations.pushPrismaSchemaToDatabase(config.db.url, config.db.shadowDatabaseUrl, prismaSchema, paths.schema.prisma, false);
    } else {
    ...
    const {
      apolloServer,
      expressServer
    } = await createExpressServer.createExpressServer(config, graphQLSchema, keystone.context);
...
async function prisma(cwd, args, frozen) {
  ...
  if (result.exitCode !== 0) throw new utils.ExitError(result.exitCode);
}
...
const start = async (cwd, _ref) => {
  ...
  // TODO: this cannot be changed for now, circular dependency with getSystemPaths, getEsbuildConfig
  const builtConfigPath = artifacts.getBuiltKeystoneConfigurationPath(cwd);

  // This is the compiled version of the configuration which was generated during the build step
  if (!fs__namespace.existsSync(builtConfigPath)) {
  ...
  if (withMigrations) {
    ...
    await migrations.deployMigrations(paths.schema.prisma, config.db.url);
  }
  console.log('✨ Connecting to the database');
  ...
  if (!((_config$ui = config.ui) !== null && _config$ui !== void 0 && _config$ui.isDisabled) || ui) {
    ...
    await nextApp.prepare();
    expressServer.use(await createExpressServer.createAdminUIMiddlewareWithNextApp(config, keystone.context, nextApp));
    console.log(`✅ Admin UI ready`);
  }
  const httpOptions = {
    port: 3000
