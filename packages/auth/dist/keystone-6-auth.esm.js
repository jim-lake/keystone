    ...
    query: {
      authenticatedItem: graphql.field({
        ...
        resolve(root, args, _ref2) {
          ...
          if ((typeof (session === null || session === void 0 ? void 0 : session.itemId) === 'string' || typeof (session === null || session === void 0 ? void 0 : session.itemId) === 'number') && typeof session.listKey === 'string') {
...
function getInitFirstItemSchema(_ref) {
  ...
  const initialCreateInput = graphql.wrap.inputObject(new GraphQLInputObjectType({
    ...createInputConfig,
...
const getSchemaExtension = _ref => {
  ...
  return graphql.extend(base => {
    ...
    const query = `query($id: ID!) { ${getGqlNames({
      listKey,
      pluralGraphQLName: ''
...
function createAuth(_ref) {
  ...
  const tokenFields = tokenType => ({
    [`${tokenType}Token`]: password({
      ...fieldConfig
    }),
    [`${tokenType}IssuedAt`]: timestamp({
      ...fieldConfig
    }),
    [`${tokenType}RedeemedAt`]: timestamp({
      ...fieldConfig
    })
  });
  const fields = {
